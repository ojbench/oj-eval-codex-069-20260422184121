...
inline MemoryBlock* allocate(int size){
    ...
    while (cur) {
        if (cur->is_free && cur->size >= size) {
            // 如果能拆分则拆分，否则整块分配
            if (cur->size >= size + static_cast<int>(sizeof(MemoryBlock))) {
                // 仅当拆分后还能容纳一个头部时才拆分
                (void)splitBlock(cur, static_cast<size_t>(size));
            }
            cur->is_free = false;
            return cur;
        }
...
inline void deallocate(MemoryBlock* ptr){
    ...
    ptr->is_free = true;
    // 向后合并相邻空闲块
    while (ptr->next && ptr->next->is_free) {
        mergeBlock(ptr);
    }
}
/**
...
inline void resetMemory(){
    ...
    head->address = 0;
    head->size = static_cast<int>(MEMORY_SIZE) - static_cast<int>(sizeof(MemoryBlock));
    head->is_free = true;
    head->next = nullptr;
}
